...
export default function Home() {
  ...
  const [sessions, setSessions] = useState<Record<string, any>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('marwa_sessions_v4');
  ...
  // Sync messages when session changes - removed the auto-greeting
  useEffect(() => {
    if (mounted) {
  ...
  // Save updates and handle scrolling
  useEffect(() => {
    if (mounted) {
      const updatedSessions = { ...sessions, [currentSessionId]: messages };
      setSessions(updatedSessions);
      localStorage.setItem('marwa_sessions_v4', JSON.stringify(updatedSessions));
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const result = await sendMessage.mutateAsync({ message: inputValue, history: history, liveOnly: false });
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: result.response }]);
    } catch (err) {
      setMessages(prev => [...prev, { id: "err", role: "error", content: "Snag detected. Try again?" }]);
    }
  };
  ...
  const handleNewSession = () => {
    ...
    setMessages([]); // Completely empty start
  };
  ...
  return (
    <div className="flex h-screen bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <aside className="w-20 md:w-64 border-r border-slate-100 flex flex-col p-4 bg-[#FCFDFE]">
        ...
        <Button onClick={handleNewSession} variant="ghost" className="w-full justify-center md:justify-start gap-3 rounded-xl py-6 mb-4 hover:bg-white hover:shadow-sm transition-all border border-transparent hover:border-slate-100 outline-none ring-0">
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-2 scrollbar-none hidden md:block">
          ...
          {Object.keys(sessions).sort().reverse().map((id) => {
            const firstUserMsg = sessions[id].find((m: any) => m.role === 'user');
            if (!firstUserMsg && id !== currentSessionId) return null; // Hide empty old sessions
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="truncate">{firstUserMsg?.content.substring(0, 20) || "New Conversation"}</span>
                </div>
              </button>
            );
          })}
        ...
        <div className="mt-auto pt-4 space-y-2 border-t border-slate-50">
          <Button variant="ghost" className="w-full justify-start gap-3 text-slate-500 hover:text-blue-600 outline-none ring-0">
          ...
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSend} className="flex items-end gap-2 p-2 bg-white border border-slate-200/80 rounded-[32px] shadow-2xl shadow-slate-200/50 focus-within:border-blue-400 transition-all outline-none">
              <textarea
                ...
                className="flex-1 bg-transparent border-none focus:ring-0 focus:outline-none px-5 py-4 text-[15px] resize-none min-h-[60px] max-h-40 outline-none"
                rows={1}
              />
              <Button type="submit" disabled={!inputValue.trim() || sendMessage.isPending} className="h-12 w-12 rounded-full bg-blue-600 hover:bg-blue-700 p-0 mb-1.5 mr-1.5 shadow-lg active:scale-95 transition-transform outline-none ring-0">
            ...
            <p className="text-[10px] text-center mt-4 text-slate-400 font-bold tracking-widest uppercase">MarwaBuddy v1.0 • Built for the Hustle</p>
          </div>
        </div>
      </main>
    </div>
