...
export default function Home() {
  const [messages, setMessages] = useState([{ id: "w", role: "assistant", content: "Hello! How can I help you with pests today?" }]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    const userMsg = { id: Date.now().toString(), role: "user" as const, content: inputValue, image: selectedImage };
    ...
    try {
      const response = await sendMessage.mutateAsync({
        message: selectedImage ? [{ type: "text", text: userMsg.content || "Identify" }, { type: "image_url", image_url: { url: selectedImage } }] : userMsg.content,
      ...
      setMessages(prev => [...prev, { id: "e", role: "error" as const, content: "Failed to connect." }]);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans text-[#1f1f1f]">
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed right-0 top-0 h-full w-72 bg-[#f8fafd] border-l z-50 p-6 shadow-xl">
            <div className="flex justify-between items-center mb-8">
              <span className="font-bold text-[#1A3D35]">Pest Intelligence</span>
            ...
            <div className="space-y-8">
              <div className="space-y-2">
                ...
                <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                  <p className="text-xs text-gray-500">System Integrity</p>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-2 text-xs font-medium">
                  <p className="text-[#4AB295]">● Instant ID</p>
                  <p className="text-[#4AB295]">● Eco-Safe Recommendations</p>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <header className="p-4 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-40">
        <div className="flex items-center gap-2 px-2">
          <Sparkles className="text-[#4AB295]" size={20} />
          <span className="font-medium text-lg tracking-tight">PestControlAI</span>
        </div>
      ...
      <main className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-4">
        <div ref={scrollRef} className="flex-1 overflow-y-auto pt-12 pb-40">
          <AnimatePresence mode="popLayout">
            {messages.length <= 1 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 text-center">
                ...
                <p className="text-gray-500 text-lg">Upload a photo or ask me about local pests.</p>
              </motion.div>
            )}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white pb-6 px-4">
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSend} className="bg-[#f0f4f9] rounded-[32px] group focus-within:bg-[#e3e8ef] transition-colors p-2">
              <AnimatePresence>
                {selectedImage && (
              ...
              <div className="flex items-center">
                ...
                <div className="flex items-center gap-1 pr-2">
                  <button type="button" onClick={() => setLocation(location ? null : {lat:0, lng:0})} className={cn("p-2 rounded-full", location ? "text-[#4AB295]" : "text-gray-400")}><Compass size={22}/></button>
                  {(inputValue || selectedImage) && (
                    <Button type="submit" disabled={sendMessage.isPending} className="rounded-full bg-[#4AB295] w-10 h-10 p-0"><Send size={18}/></Button>
                  )}
                </div>
              </div>
            </form>
            <p className="text-[10px] text-center mt-3 text-gray-400">PestControlAI provides eco-safe advice. Always verify safety labels.</p>
          </div>
        </div>
      </main>
    </div>
