...
export default function Home() {
  ...
  // PERSISTENT MEMORY: Load from localStorage on startup
  const [messages, setMessages] = useState<Array<{ id: string; role: "user" | "assistant" | "error"; content: string }>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('marwa_chat_v1');
      return saved ? JSON.parse(saved) : [{ id: "init", role: "assistant", content: "Hey! I'm MarwaBuddy. What's on your mind today?" }];
    }
    return [{ id: "init", role: "assistant", content: "Hey! I'm MarwaBuddy. What's on your mind today?" }];
  });
  ...
  // AUTO-SAVE: Save every message to localStorage
  useEffect(() => {
    localStorage.setItem('marwa_chat_v1', JSON.stringify(messages));
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    // Add history support so the AI has context
    ...
    try {
      // Sending currentInput + previous history to the API
  ...
  const startNewSession = () => {
    const freshState = [{ id: "init", role: "assistant", content: "New session started. What's the plan, Osteen?" }];
    setMessages(freshState);
    localStorage.removeItem('marwa_chat_v1');
  };
    ...
    <div className="flex h-screen bg-white text-slate-900 font-sans overflow-hidden">
      ...
      <aside className="w-20 md:w-64 border-r border-slate-100 flex flex-col p-4 bg-[#FCFDFE]">
        <div className="flex items-center gap-3 px-2 mb-8">
          ...
          <h1 className="font-bold text-lg tracking-tight hidden md:block text-slate-800">MarwaBuddy</h1>
        </div>

        <Button 
          onClick={startNewSession}
          variant="ghost" 
          className="w-full justify-center md:justify-start gap-3 rounded-xl py-6 mb-2 hover:bg-white hover:shadow-sm transition-all border border-transparent hover:border-slate-100 outline-none ring-0 focus:ring-0 focus:outline-none"
        >
        ...
        <div className="mt-auto space-y-2">
          <Button variant="ghost" size="icon" className="w-full md:w-auto md:px-3 md:justify-start gap-3 text-slate-500 hover:text-blue-600 outline-none ring-0 focus:ring-0 focus:outline-none">
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center gap-3 px-2">
            ...
            <div className="hidden md:block overflow-hidden">
              ...
              <p className="text-[10px] text-blue-500 font-medium uppercase tracking-wider">Dev Mode</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col relative bg-[#F8F9FA]">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-12 pt-12 pb-40 scrollbar-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {MARWA_SUGGESTIONS.map((s, i) => (
                    <motion.button 
                      ...
                      className="p-6 bg-white border border-slate-200/60 rounded-3xl text-left hover:border-blue-400 hover:shadow-xl hover:shadow-blue-500/5 transition-all group outline-none ring-0 focus:outline-none"
                    >
          ...
          <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              <Button 
                ...
                className="h-12 w-12 rounded-full bg-blue-600 hover:bg-blue-700 p-0 mb-1.5 mr-1.5 shadow-lg shadow-blue-600/20 active:scale-90 transition-transform outline-none ring-0 focus:ring-0"
              >
            ...
            <p className="text-[10px] text-center mt-4 text-slate-400 font-bold tracking-widest uppercase pointer-events-none">
              MarwaBuddy v1.0 • Build #2026
            </p>
          </div>
        </div>
      </main>
